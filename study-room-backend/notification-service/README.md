...
Отправка писем теперь выполняется в фоне: сервис сразу принимает запрос и не ждёт длительной SMTP-сессии. Ошибки доставки записываются в статус уведомления на `failed`, но не блокируют HTTP-обработчик.
