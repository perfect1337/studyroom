...
func NewRouter(d *Deps) http.Handler {
	...
	// Отдельный лимит регистрации: не более 2 регистраций в минуту с одного IP.
	registrationLimiter := middleware.NewIPRateLimiter(2, time.Minute)
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.With(middleware.RateLimit(registrationLimiter)).Post("/auth/register", authHandler.Register)
