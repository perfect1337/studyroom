// Package userclient — единственный синхронный HTTP-вызов Contracts Service
// к User Service. Всё остальное (роль, филиал) читается из локального кэша
// user_refs, наполняемого событиями (см. internal/events).
//
// Список "чьи я дети" — это связь parent↔student, которой в Contracts
// Service нет и не будет (см. service-info/microservices-plan.md:
// "database-per-service", у Contracts нет причин дублировать семейные
// связи). Поэтому для GET /contracts/{id}/expiry с role=parent сервис один
// раз синхронно спрашивает User Service — см. api-contracts.md, 1.18
...
// Bearer-токен, что пришёл в запрос к Contracts Service (проброс авторизации
