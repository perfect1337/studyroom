...
const defaultTestDSN = "postgres://academic_service:devpassword123@localhost:5435/study_room_academic?sslmode=disable"
...
func TestMain(m *testing.M) {
	...
	reportFile = f
	writeReport("Study Room — academic-service contract tests\n")
...
func getEnv(t *testing.T) *env {
	...
	sharedOnce.Do(func() {
		...
		if err != nil {
			sharedErr = fmt.Errorf("connect TEST_DATABASE_URL: %w (start postgres: docker compose up -d postgres-academic)", err)
			return
		}
...
// сервис — Academic Service не выпускает токены сам (это делает User
...
type apiResult struct {
	...
	Body   map[string]any
	Raw    []byte
	// Location — заголовок Location из ответа (для проверки 302-редиректа
	// в GET /homework/{id}/open, см. api-contracts.md 2.14).
	Location string
}
...
func (e *env) request(method, path string, body any, headers map[string]string) apiResult {
	...
	raw := rr.Body.Bytes()
	out := apiResult{Status: rr.Code, Raw: raw, Body: map[string]any{}, Location: rr.Header().Get("Location")}
	if len(raw) > 0 {
	...
	if out.Location != "" {
		e.logf("│    location: %s", out.Location)
	}
	if len(raw) > 0 {
...
// дробной части — например, для подстановки в URL вида /courses/{id}.
