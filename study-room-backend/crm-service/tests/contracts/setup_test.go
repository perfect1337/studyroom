...
const defaultTestDSN = "postgres://crm_service:devpassword123@localhost:5436/study_room_crm?sslmode=disable"

const testJWTSecret = "test-jwt-secret-for-contracts"
const testWebhookSecret = "test-tilda-webhook-secret"
...
func TestMain(m *testing.M) {
	...
	reportFile = f
	writeReport("Study Room — crm-service contract tests\n")
...
func getEnv(t *testing.T) *env {
	...
	sharedOnce.Do(func() {
		...
		if err != nil {
			sharedErr = fmt.Errorf("connect TEST_DATABASE_URL: %w (start postgres: docker compose up -d postgres-crm)", err)
			return
		}
		...
		deps := app.NewDeps(pool, tm, testWebhookSecret, events.NoopPublisher{})
...
// сервис — CRM Service не выпускает токены сам (это делает User Service),
// поэтому в тесте подписываем вручную через тот же auth.Claims.
...
// signWebhook считает HMAC-SHA256(secret, body) в hex — то, что ожидает
// заголовок X-Tilda-Signature (см. handlers/application_handler.go).
...
// requestRaw — как request, но без JSON-маршалинга тела (нужно для вебхука,
// где мы сами считаем подпись от сырых байт).
func (e *env) requestRaw(method, path string, rawBody []byte, headers map[string]string) apiResult {
	e.t.Helper()
...
// дробной части — например, для подстановки в URL вида /applications/{id}.
