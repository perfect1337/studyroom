...
func (r *UserRefRepository) Upsert(ctx context.Context, u *models.UserRef) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO user_refs (user_id, full_name, role, branch_id, branch_ids, class_info, synced_at)
		...
		ON CONFLICT (user_id) DO UPDATE SET
			...
			class_info = COALESCE(EXCLUDED.class_info, user_refs.class_info),
			synced_at = now()`,
		u.UserID, u.FullName, u.Role, u.BranchID, u.BranchIDs, u.ClassInfo)
	return err
}

func (r *UserRefRepository) GetByID(ctx context.Context, id int64) (*models.UserRef, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT user_id, full_name, role, branch_id, branch_ids, class_info FROM user_refs WHERE user_id = $1`, id)

	var u models.UserRef
	err := row.Scan(&u.UserID, &u.FullName, &u.Role, &u.BranchID, &u.BranchIDs, &u.ClassInfo)
	if err != nil {
...
func (r *UserRefRepository) FindBranchOwner(ctx context.Context, branchID int64) (*models.UserRef, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT user_id, full_name, role, branch_id, branch_ids FROM user_refs
