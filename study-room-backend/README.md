...
Это интеграционные тесты, они требуют поднятых сервисов и NATS. Перед запуском выполните сервисы:
...
После выполнения тестов будет создан файл `tests/load/report.md` с понятным отчётом по каждому сценарию: результат, длительность, важные шаги и обнаруженные проблемы.

Если сервисы недоступны, тесты теперь падают с ошибкой, а `tests/load/report.md` фиксирует, какие сервисы или инфраструктура не были доступны.

Для локального запуска из корня репозитория можно также использовать:
