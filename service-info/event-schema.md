...
| `branch_id` | int64 | да — `Contract.branch_id`, филиал, чей руководитель выдал договор (**не** домашний филиал ученика). Становится `enrollments.branch_id` (см. `academic-service`, `CreateFromContract`) — так ученик из одного филиала может быть зачислен на курс, договор по которому оформлен в другом филиале, и этот филиал сможет управлять зачислением (назначать репетитора и т.д.). Если поле отсутствует (событие от старой версии издателя), Academic Service откатывается на домашний филиал ученика — временная защита обратной совместимости. |
