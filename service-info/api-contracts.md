...
`auth`: true
`roles`:
- `PATCH` — `owner`, `branch_owner` (курс общий для всей сети, редактировать может любой из них)
- `DELETE` — `owner` **только** (`branch_owner` не может удалять курсы — удаление затрагивает всю сеть, а не только его филиал)
