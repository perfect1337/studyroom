...
## 1.9. Список пользователей
`GET /users?role=tutor&branch_id=3&page=1`

Тело запроса: query-параметры (`role`, `branch_id`, `search`, `page`)
Тело ответа:
```json
{ "items": [ { "id": 1, "role": "tutor", "first_name": "...", "..." : "..." } ], "total": 48, "page": 1 }
```
`auth`: true
`roles`: `owner` (без ограничения по `branch_id`), `branch_owner` (сервер принудительно подставляет его `branch_id`, игнорируя параметр в запросе)
...
```
`auth`: true
`roles`: `owner` (все филиалы), `branch_owner` (только свой — 1 элемент в массиве)
...
```
`auth`: true
`roles`: `branch_owner` (только договоры своего филиала), `parent` и `tutor`(только договоры своих детей) — **никаких других полей (сумма, статус оплаты) в ответе нет, только дата окончания и начала договора**
